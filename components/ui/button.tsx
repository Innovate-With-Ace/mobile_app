...
const buttonVariants = cva(
  ...
  ),
  {
    variants: {
      ...
      size: {
        // Native heights stay >= 44pt (DESIGN.md min touch target) regardless
        // of the sm:* web breakpoints, which only apply on web.
        default: cn('h-11 px-4 py-2 sm:h-9', Platform.select({ web: 'has-[>svg]:px-3' })),
        sm: cn('h-11 gap-1.5 rounded-xl px-3 sm:h-8', Platform.select({ web: 'has-[>svg]:px-2.5' })),
